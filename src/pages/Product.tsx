...
const ProductPage = () => (
  <Layout>
    {/* Hero */}
    <section className="relative overflow-hidden py-20 sm:py-28 md:py-36" style={{ background: "linear-gradient(135deg, #060d1f 0%, #0a1628 50%, #06111e 100%)" }}>
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[500px] sm:w-[700px] h-[300px] sm:h-[400px] rounded-full opacity-20" style={{ background: "radial-gradient(ellipse, #3b82f6 0%, transparent 70%)", filter: "blur(60px)" }} />
        <div className="absolute bottom-0 right-0 w-[300px] sm:w-[400px] h-[200px] sm:h-[300px] opacity-10" style={{ background: "radial-gradient(ellipse, #06b6d4 0%, transparent 70%)", filter: "blur(50px)" }} />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-3xl">
          <span className="text-blue-400 text-xs sm:text-sm font-semibold tracking-widest uppercase mb-4 sm:mb-6 block">Product</span>
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-semibold text-white leading-tight mb-6 sm:mb-8" style={{ textShadow: "0 2px 20px rgba(0,0,0,0.4)" }}>
    ...
    <Section className="section-dark">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-lg sm:rounded-xl overflow-hidden border border-border/10 glow-md">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 mt-12 sm:mt-16">
        {[
          ...
          <motion.div key={i} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center card-dark p-6 sm:p-8">
            <p className="text-2xl sm:text-3xl font-bold text-hero-foreground">{m.value}</p>
            <p className="text-xs text-primary mt-1">{m.change}</p>
            <p className="text-xs sm:text-sm text-surface-dark-foreground mt-1">{m.label}</p>
          </motion.div>
        ))}
      </div>
    </Section>
    ...
    <Section className="section-dark">
      <div className="text-center max-w-2xl mx-auto px-4">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-semibold text-hero-foreground mb-4 sm:mb-6">See OptiMerce AI in action</h2>
        <p className="text-base sm:text-lg text-surface-dark-foreground mb-8 sm:mb-10">Book a demo to explore how our platform can transform your operations.</p>
        <Link to="/contact" className="inline-flex items-center justify-center gap-2 px-8 py-3 rounded-lg bg-primary text-primary-foreground font-semibold hover:opacity-90 transition-opacity tap-highlight-transparent">
          Book a Demo <ArrowRight size={18} className="hidden sm:block" />
        </Link>
      </div>
    </Section>
  </Layout>
